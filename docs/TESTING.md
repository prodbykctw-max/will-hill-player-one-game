...
also seeds `wh_intro_v4`, so its callers get stage one without the frozen
...
`wh_intro_v4`.** Stage one opens frozen on Will Hill's intro bubbles until
...
`localStorage.setItem('wh_intro_v4', '1')` next to its `goto`.
...
| `daynight` | day and night variants of all five stages differ where they should |
...
| `tutorial` | the stage-one intro: it opens only once he has LANDED (the first cut froze him mid-drop), the world is frozen while it is up, a line types out and the first press finishes it, the second turns the page, a real JUMP press reaches it, P cannot pause over it, the bag / enemy / champagne pictures sit on the lines that name them, it is eight cards — greeting and goal on one, one JUMP/manholes card, the champagne power-up card, "Let's get it!" last — it closes after the last one and sets `wh_intro_v4`, nothing opens mid-stage afterwards, the move card's arrows are drawn (no typed ◀ ▶, which a phone turns into blue emoji), and a profile holding the retired `wh_howto_seen`, `wh_intro_seen`, `wh_intro_v2` and `wh_intro_v3` latches still gets it |
...
- **`bankline` check 4 ("with nothing banked the whole pocket still comes
  out") fails about half the time — on `main` as well, so it is not any
  recent change.** Measured 2026-09-25: 3 of 6 runs red on main `0ee6543`,
  the same rate on the branch after it. The failing runs read
  `{"hearts":2,"score":200,"banked":0}`. The pocket did scatter, and the
  bank is correctly 0, but he re-caught TWO bags on the knockdown tick,
  where the check tolerates one. Where the scattered bags land depends on
  the random level, so the root cause is in the check's tolerance or its
  snapshot timing, not in the bank/pocket rule. Checks 1–3 (the ones that
  grade the bank) stay green on every run. Not yet fixed. The fix is an
  exact snapshot at the hit, not a wider tolerance.
