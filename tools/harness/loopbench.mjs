...
// What happened: he trimmed the intro by ear, the readout called it 15.829
// bars in RED because the DETECTED tempo was 132.51, and he stopped to ask
// whether the tool would even accept what he had picked. It was 16.000 bars
// exactly at 134. The interface implied his good work was suspect.
//
// Graded on stage_02 rather than on his original numbers, because those no
// longer exist to reproduce: his choice SHIPPED, so the title clip now IS his
// 28.657s loop. The behaviour is what must not regress, and it is only
// testable on a cue whose tempo is still a guess.
await p.uncheck('#snap');
await p.fill('#endN', '61.400'); await p.dispatchEvent('#endN', 'change');
...
check('a length that is not whole bars at the detected tempo is flagged',
  fit.red && /\d+\.\d+ bars/.test(fit.len), fit.len);
check('and the tempo that WOULD make it whole is offered, rather than left suspect',
  fit.shown && fit.chips.length >= 1, fit.chips.join(' · '));
...
check('tapping it turns the same length green on a whole bar count',
...
const intro = await p.evaluate(() => ({
  dur: window.__bench.source().duration,
  bpm: document.getElementById('bpm').value,
  guessed: !document.getElementById('bpmWarn').hidden,
  len: document.getElementById('lenOut').textContent,
  green: document.getElementById('lenOut').className.includes('ok'),
  numbers: JSON.parse(window.__bench.numbers()),
}));
check('the intro that ships is his 16 bars at 134, exactly',
  near(intro.dur, 28.657, 0.01) && /16\.000 bars/.test(intro.len) && intro.green,
  `${intro.dur.toFixed(3)}s — ${intro.len}`);
