...
// Defaults to `shots/` (gitignored), never the repo root — the same fix the
// other harnesses got after a sweep of them left PNGs and JSON lying beside
// the source. This file was written during that sweep and missed it.
...
const film = await p.evaluate(async () => {
  ...
  await frame();
  const out = [];
  // ⚠️ ASK title.js WHEN IT FINISHES, DO NOT GUESS AND DO NOT INVENT AN API.
  // This file was written against a `__title.settledAt()` that was planned for
  // the three-beat intro and never shipped, so it threw on the first run
  // against the real page. INTRO_TICKS is the constant title.js actually
  // exports and main.js actually reads, so grading against it is grading the
  // same number the game uses rather than a harness-only copy of it.
  const { INTRO_TICKS } = await import('/src/render/title.js');
  const end = INTRO_TICKS + 8;
  // ⚠️ THERE IS NO `state.introT`. It is a LOCAL inside draw(), computed as
  // screenT - introAt, so `g.introT` reads undefined — and `(g.introT || 0)`
  // then pins the clock at 0 and the loop below never ends. The first run of
  // this file hung for four minutes on exactly that. Derive it the same way
  ...
  while (introT() < end) {
    ...
    out.push({ t: introT(), wy: w.y, wn: w.n, ly: l.y, ln: l.n });
    await frame();
  }
  return { rows: out, boxes, settledAt: INTRO_TICKS };
});
...
check('there is a visible beat between them',
  wm !== null && lg !== null && (lg - wm) >= 12, `${lg - wm} ticks apart`);
