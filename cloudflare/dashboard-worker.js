 ...
 *{box-sizing:border-box}
 ...
 body{margin:0 auto;max-width:960px;background-color:#0a0810;color:var(--fg);
   ...
   /* ⚠️ THE RAILS ARE A FIXED WIDTH, NOT A PERCENTAGE, AND THAT IS THE WHOLE
      POINT. Sized at their true 10.8% of the plate they ate 22% of every
      screen, and on a phone that left a 330px content column — the client:
      "why is it so tall and skinny bro that's not gonna fit". His cab is a
      phone-shaped painting; the page inside it is a data table. Squeezing a
      26px strip of vertical metal is invisible, and it hands ~100px back to
      the content on a 430px screen.
      max-width caps the frame so a laptop gets a centred cab rather than a
      2m-wide console, and the bottom cap scales with the frame, not the
      window. */
   background-size:26px auto,26px auto,100% auto,100% auto;
   padding:6px 34px calc(min(960px,100vw) * 0.21);}
 @media (max-width:520px){ body{background-size:18px auto,18px auto,100% auto,100% auto;
   padding:4px 24px calc(100vw * 0.21)} }
 ...
 .strip{display:grid;grid-template-columns:repeat(auto-fit,minmax(96px,1fr));gap:8px;padding:12px 0 0}
